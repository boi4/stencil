...
void stencil(const size_t nx, const size_t ny, const size_t width, const size_t height,
             float* image, const size_t niters);
...
int main(int argc, char* argv[])
{
  ...
  double tic = wtime();
  stencil(nx, ny, width, height, image, 2*niters);
  double toc = wtime();
             ...
             float* image, const size_t niters)
{
  ...
  float * restrict row_buffer1 = malloc(nx * sizeof(float));
  float * restrict row_buffer2 = malloc(nx * sizeof(float));
  float * restrict row_buffer3 = malloc(nx * sizeof(float));
